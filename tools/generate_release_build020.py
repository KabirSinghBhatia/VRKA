"""Authoritative Release Builder and Packager for VRKA 4.5.2 Build 020.

Produces a self-contained, build-specific release directory:
  releases/VRKA-4.5.2-Build-020/

All 8 Official Release Assets:
  - VRKA-4.5.2-Build-020-Portable.exe
  - VRKA-4.5.2-Build-020-Portable.zip
  - VRKA-4.5.2-Build-020-Setup.exe
  - VRKA-4.5.2-Build-020-Source.zip
  - SHA256SUMS.txt
  - SHA256SUMS.txt.asc
...
RELEASE_DIR = PROJECT_ROOT / "releases" / "VRKA-4.5.2-Build-020"
PORTABLE_SOURCE = PROJECT_ROOT / "VRKA-portable"

# Target filenames
FN_PORTABLE_EXE = "VRKA-4.5.2-Build-020-Portable.exe"
FN_PORTABLE_ZIP = "VRKA-4.5.2-Build-020-Portable.zip"
FN_SETUP_EXE    = "VRKA-4.5.2-Build-020-Setup.exe"
FN_SOURCE_ZIP   = "VRKA-4.5.2-Build-020-Source.zip"
...
def create_source_zip(dest_zip: Path) -> None:
    ...
    ignore_dirs = {
        ...
        ".release_stage_4_0_1", ".release_stage_4_5_0", ".release_stage_4_5_1", ".release_stage_4_5_2"
    }
    ...
    with zipfile.ZipFile(dest_zip, "w", zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(PROJECT_ROOT):
            ...
            for file in files:
                ...
                zf.write(file_path, arcname=str(Path("VRKA-4.5.2-source") / rel_file))
...
def generate_release_package(source_only: bool = False, preserve_packages: bool = False) -> None:
    print("=" * 70)
    print("VRKA 4.5.2 BUILD 020 — AUTHORITATIVE RELEASE PACKAGING PIPELINE")
    print("=" * 70)
        ...
        with zipfile.ZipFile(dest_portable_zip, "w", zipfile.ZIP_DEFLATED) as zf:
            for root, _, files in os.walk(PORTABLE_SOURCE):
                for f in files:
                    ...
                    zf.write(fp, arcname=str(Path("VRKA-4.5.2-portable") / rel))
    ...
    build_report_content = f"""# VRKA 4.5.2 Build 020 — Authoritative Build Report
...
- **Release Directory**: `releases/VRKA-4.5.2-Build-020`
- **Portable EXE**: `releases/VRKA-4.5.2-Build-020/{FN_PORTABLE_EXE}`
- **Portable ZIP**: `releases/VRKA-4.5.2-Build-020/{FN_PORTABLE_ZIP}`
- **Installer**: `releases/VRKA-4.5.2-Build-020/{FN_SETUP_EXE}`
- **Source ZIP**: `releases/VRKA-4.5.2-Build-020/{FN_SOURCE_ZIP}`
- **SHA256SUMS**: `releases/VRKA-4.5.2-Build-020/{FN_SHA256SUMS}`
- **SHA256 Signature**: `releases/VRKA-4.5.2-Build-020/{FN_SHA256_ASC}`
- **Release Manifest**: `releases/VRKA-4.5.2-Build-020/{FN_MANIFEST_JSON}`
- **Manifest Signature**: `releases/VRKA-4.5.2-Build-020/{FN_MANIFEST_ASC}`

## Metadata
- **Application**: VRKA
- **Display Version**: 4.5.2
- **Build Number**: 020
...
"""
    ...
    test_results_content = f"""# VRKA 4.5.2 Build 020 — Automated Test Suite Results
...
"""
    ...
    sec_review_content = f"""# VRKA 4.5.2 Build 020 — Security Architecture & Release Verification

## 1. Release Authenticity & Trust Model

VRKA 4.5.2 implements an authenticated OpenPGP release verification model:
...
All release files in `releases/VRKA-4.5.2-Build-020/` were cryptographically signed:
...
"""
    ...
    checkpoint_content = f"""# VRKA 4.5.2 Build 020 — Windows Release Checkpoint
...
"""
    ...
    print("\n" + "=" * 70)
    print("VRKA 4.5.2 BUILD 020 PACKAGING PIPELINE COMPLETE")
    print("=" * 70)
